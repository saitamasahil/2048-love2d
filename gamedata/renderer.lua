...
function renderer.captureOldTutorialSlide(page)
    ...
    if not tutorial_old_canvas or tutorial_old_canvas:getWidth() ~= w or tutorial_old_canvas:getHeight() ~= h then
...
function renderer.captureOldAchievementsSlide(tab)
    local w, h = love.graphics.getDimensions()
    if not achievements_old_canvas or achievements_old_canvas:getWidth() ~= w or achievements_old_canvas:getHeight() ~= h then
...
function renderer.isKeyboardMode()
    local mode = _G.controls_help or "auto"
    if mode == "auto" then
        return (_G.input_mode or "keyboard") == "keyboard"
    end
...
function renderer.getButtonPrompt(key)
    ...
    elseif renderer.isKeyboardMode() then
        ...
        return keyboard_mapping[key] or key
    end
    return key
end
...
function renderer.formatText(text)
    ...
    elseif renderer.isKeyboardMode() then
        text = text:gsub("Press B", "Press Backspace")
        text = text:gsub("%[B%]", "[Backspace]")
        text = text:gsub("Press L1", "Press Q")
        text = text:gsub("%[L1%]", "[Q]")
        text = text:gsub("Press R1", "Press E")
        text = text:gsub("%[R1%]", "[E]")
        text = text:gsub("Press Y", "Press Y")
        text = text:gsub("%[Y%]", "[Y]")
        text = text:gsub("B button", "Backspace key")
    end
    return text
end
...
function renderer.startThemeTransition(drawTarget)
    ...
    local w, h = love.graphics.getDimensions()
    if not transition_canvas or transition_canvas:getWidth() ~= w or transition_canvas:getHeight() ~= h then
...
function renderer.drawTutorial(page, skip_transition, static_only)
    ...
    if _G.tutorial_slide_timer and _G.tutorial_slide_timer > 0 then
        ...
        if not _G.tutorial_slide_ready then
            if not tutorial_new_canvas or tutorial_new_canvas:getWidth() ~= w or tutorial_new_canvas:getHeight() ~= h then
...
function renderer.getSettingsOptions()
    ...
    local help_mode = _G.controls_help or "auto"
    local help_lbl = "Control Hints: " .. (help_mode == "auto" and "Auto" or (help_mode == "gamepad" and "Gamepad" or "Keyboard"))
...
function renderer.drawSettings(selection, skip_transition)
    ...
    local gap = (_G.text_size == "large" and 33 or 30) * scale
    ...
    local header_h = math.floor((_G.text_size == "large" and 60 or 65) * scale)
...
function renderer.drawAchievements(scroll, skip_transition, static_only, override_tab)
    ...
    if not static_only and _G.achievements_slide_timer and _G.achievements_slide_timer > 0 then
        ...
        if not _G.achievements_slide_ready then
            if not achievements_new_canvas or achievements_new_canvas:getWidth() ~= w or achievements_new_canvas:getHeight() ~= h then
    ...
    if love.system.getOS() ~= "Web" then
        ...
        local lr_text = renderer.getButtonPrompt("L/R")
        local lr_w = math.max(math.floor(34 * scale), font_help_key:getWidth(lr_text) + math.floor(12 * scale))
        drawKeyBadge("L/R", left_x, badge_y + (badge_h - dpad_size) / 2, lr_w, dpad_size)
        left_x = left_x + lr_w + math.floor(6 * scale)
        love.graphics.setFont(font_help_label)
        love.graphics.setColor(ui_text)
