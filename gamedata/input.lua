...
function love.keypressed(key)
    ...
    if _G.controls_help == "auto" then
        _G.input_mode = "keyboard"
    end

    local translated = key
    if love.system.getOS() ~= "Web" then
        if key == "z" or key == "q" then
            translated = "l1"
        elseif key == "c" or key == "e" then
            translated = "r1"
        end
    end

    for event_name, k in pairs(input.events) do
        if translated == k or (is_web and key == "backspace" and k == "escape") then
            input.state[translated] = true
            emit(k, false)
        end
    end
