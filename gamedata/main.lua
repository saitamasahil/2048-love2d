...
local function captureOldScreen()
    ...
    if not old_screen_canvas or old_screen_canvas:getWidth() ~= w or old_screen_canvas:getHeight() ~= h then
...
function love.load(args)
    ...
    -- Initialize save system (standard OS save folder on PC, local static/ on Web)
    ...
    _G.controls_help = save.loadControlsHelp() or "auto"
        ...
        elseif _G.appState == "SETTINGS" then
            ...
            elseif event == input.events.CONFIRM then
                ...
                elseif sel:match("^CRT Shader") then
                    ...
                    sound.playMenuSelect()
                elseif sel:match("^Control Hints") then
                    sound.playMenuSelect()
                    if _G.controls_help == "auto" then
                        _G.controls_help = "gamepad"
                    elseif _G.controls_help == "gamepad" then
                        _G.controls_help = "keyboard"
                    else
                        _G.controls_help = "auto"
                    end
                    save.saveControlsHelp(_G.controls_help)
...
function love.draw()
    ...
    local function draw_internal()
        ...
        if screen_transition_timer > 0 then
            ...
            if not screen_canvas or screen_canvas:getWidth() ~= w or screen_canvas:getHeight() ~= h then
                screen_canvas = love.graphics.newCanvas(w, h)
            end
